    ...
    for (unsigned short i = 0; i < count; i++) {
        ...
        if (is_passable(tile)) {
            ...
            if (is_passableup && is_passabledown) {
                _all_pushes.insert_edge(static_cast<index_t>(i - width), i);
                _all_pushes.insert_edge(static_cast<index_t>(i + width), i);
            }
            if (is_passableleft && is_passableright) {
                _all_pushes.insert_edge(static_cast<index_t>(i - 1), i);
                _all_pushes.insert_edge(static_cast<index_t>(i + 1), i);
            }

            if (is_passableup)    { _all_moves.insert_edge(i, static_cast<index_t>(i - width)); }
            if (is_passableleft)  { _all_moves.insert_edge(i, static_cast<index_t>(i - 1)); }
            if (is_passableright) { _all_moves.insert_edge(i, static_cast<index_t>(i + 1)); }
            if (is_passabledown)  { _all_moves.insert_edge(i, static_cast<index_t>(i + width)); }
        }
    }
...
vector<PushInfo> Board::possible_pushes() const {
    ...
    for (const auto ibox: _boxes) {
        for (auto it = _all_pushes.edges_begin(ibox); it != _all_pushes.edges_end(); ++it) {
            ...
            /* if (boxbits[ibox_dest] || boxbits[iplayer_dest]) { continue; } */
