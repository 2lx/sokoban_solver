...
bool check_solution(const string & level) {
...
bool operator<(const Coords & l, const Coords & r) {
    return l.dy < r.dy || (l.dy == r.dy && l.dx < r.dx);
}

bool operator==(const Coords & l, const Coords & r) {
    return l.dy == r.dy && l.dy == r.dy;
}


ostream & operator<<(ostream & str, const Coords & c) {
    return str << "{" << setw(2) << c.dx << ", " << setw(2) << c.dy << "}";
}

Coords get_coords(size_t goali, size_t roomi, size_t width) {
    /* cout << "COORDS: " << goali << ' ' << roomi << ' ' << width << endl; */
    Coords c;
    c.dx = static_cast<int>(roomi % width) - static_cast<int>(goali % width);
    c.dy = static_cast<int>(roomi / width) - static_cast<int>(goali / width);
    /* cout << c.dx << ' ' << c.dy << endl; */
    return c;
}
...
struct DeadlockInfo {
    ...
    std::vector<Coords> boxes;
    std::vector<Coords> goals;
    bool independent_of_goals;
};

bool operator==(const DeadlockInfo & l, const DeadlockInfo & r) {
    return l == r;
}

vector<DeadlockInfo> get_deadlocks() {
    vector<DeadlockInfo> result;

    string level_pattern = 1 + R"(
########
#1     #
#      #
#  ??  #
...
########
    ...
    const size_t goalfi = distance(begin(level_pattern),
                                         find(begin(level_pattern), end(level_pattern), '1'));
    const size_t boxfi = distance(begin(level_pattern),
                                        find(begin(level_pattern), end(level_pattern), '2'));
    const size_t testi = distance(begin(level_pattern),
                                        find(begin(level_pattern), end(level_pattern), '$'));
    const size_t width = distance(begin(level_pattern),
                                        find(begin(level_pattern), end(level_pattern), '\n')) + 1;
        ...
        for (size_t boxbits = 0; boxbits < (1u << box_all_indexes.size()); boxbits++) {
            ...
            for (size_t goalbits = 0; goalbits < goal_permutation_max; goalbits++) {
                ...
                if (!check_solution(level_pattern)) {
                    ...
                    auto getc = [testi, width](size_t ind) {
                        return get_coords(testi, ind, width);
                    };
...
int main() {
    auto dli = get_deadlocks();

    cout << "{\n";
    for (const auto & [ws, bs, gs, flag]: dli) {
        string comm = "//   \n//  ?\n";
        for (auto [dx, dy]: ws) {
            comm[10 + dx + dy*6] = '#';
        }
